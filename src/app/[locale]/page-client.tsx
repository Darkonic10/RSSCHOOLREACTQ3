...
interface Props {
  query: string;
  page: number;
  searchResults: AnimeSearchResponse | null;
  detailsError: string | null;
  details?: number;
  initialDetails?: AnimeData | null;
}

export default function HomePageClient({ query, page, details, searchResults: initialSearchResults, detailsError, initialDetails }: Props) {
  const searchParams = useSearchParams();
  const router = useRouter();
  const queryClient = useQueryClient();

  const { data: searchResults, isLoading } = useSearchAnime(query, page, initialSearchResults);
    ...
    <>
      <main className={styles.main}>
        <div className={styles.resultsContainer}>
          ...
          <CustomButton onClick={handleRefetchAll}>{t('invalidate')}</CustomButton>
        </div>

        {details && (
          <div className={styles.detailsContainer}>
            <AnimeDetailsComponent initialDetails={initialDetails} detailsError={detailsError} />
          </div>
        )}
      </main>
      <SelectedFooter />
